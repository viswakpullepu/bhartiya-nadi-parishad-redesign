...
export default function LeadershipSection() {
  return (
    <section id="leadership" className="py-24 md:py-32 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-base text-[#425B76] mt-4 leading-relaxed font-normal">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {leaders.map((person, idx) => (
            <SpotlightCard key={idx} className="p-6 text-left flex flex-col justify-between group">
              <div>
                {/* Portrait */}
                <div className="relative rounded-2xl overflow-hidden aspect-[4/3] mb-5 bg-slate-100 border border-gray-100">
                  <img
                    src={person.image}
                    alt={person.name}
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="px-2.5 py-1 rounded-full bg-white/90 backdrop-blur-md text-[10px] font-bold uppercase tracking-wider text-[#059669] shadow-sm">
                      {person.tag}
                    </span>
                  </div>
                </div>

                <h3 className="font-serif text-xl font-bold text-[#0A1D33] leading-snug group-hover:text-[#1C71C6] transition-colors">
                  {person.name}
                </h3>
                <p className="text-xs font-semibold text-[#10B981] mt-0.5 mb-2">
              ...
              <div className="pt-4 mt-4 border-t border-gray-100 flex items-center justify-between text-xs text-[#6E859E]">
