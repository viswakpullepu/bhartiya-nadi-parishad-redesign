...
export default function PublicationsCatalog() {
  ...
  return (
    <section id="publications" className="py-24 md:py-32 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6 text-left">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#E0F2FE] text-[#0284C7] text-xs font-semibold uppercase tracking-wider mb-3">
        ...
        {/* Filter Controls (Pine Labs pill navigation style) */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-10 pb-6 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 no-scrollbar">
            {categories.map((cat) => (
              <button
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-full text-xs sm:text-sm font-semibold transition-all duration-300 ${
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-sm rounded-full bg-[#F6FAFC] border border-[#2B8EE8]/20 focus:outline-none focus:border-[#2B8EE8] focus:bg-white transition-colors"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((doc, idx) => (
            <SpotlightCard key={idx} className="p-6 text-left flex flex-col justify-between group">
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className="px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider bg-[#ECFDF5] text-[#059669]">
                    {doc.badge}
                  </span>
                  <span className="text-xs font-mono text-gray-400">{doc.size}</span>
                </div>

                <h3 className="font-serif text-lg font-bold text-[#0A1D33] group-hover:text-[#1C71C6] transition-colors line-clamp-2 mb-2">
                  {doc.title}
                </h3>
              ...
              <div className="pt-4 border-t border-gray-100 flex items-center justify-between">
                <span className="text-[11px] font-medium text-[#6E859E]">
                  {doc.category}
                </span>
        ...
        {filtered.length === 0 && (
          <div className="py-16 text-center text-gray-500">
            <p className="text-base font-medium">No publications found matching "{searchQuery}".</p>
          </div>
        )}
